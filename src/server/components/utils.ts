...
//         const fd: any = {}
// 	fd.id ="00008101-000A38163AE9001E";
// 	fd.name="iPad";
//        	fd.type="remote";
//         unique.push(fd)
// 	return unique;
...
export async function findDevices() {
  const devieObj = await frida.getUsbDevice({ timeout: null });
  const unique = Array();
  const deviceJson = {
    id: devieObj.id,
    name: devieObj.name,
...
export async function findProcesses(deviceId: string, appName: string) {
  ...
  if (appName.trim() !== "") {
    foundProcess = processes.filter((proc) => {
