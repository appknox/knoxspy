...
exports.timeoutFunc = exports.attachApp = exports.startApp = exports.findApps = exports.findProcesses = exports.findDevices = exports.parseDevices = exports.split_by_length = exports.bytesToImageURI = void 0;
const child = __importStar(require("child_process"));
const frida = __importStar(require("frida"));
const device_1 = require("frida/dist/device");
function bytesToImageURI(byteArray) {
    const data = Buffer.from(byteArray).toString('base64');
    const mimeType = 'image/png';
    const dataURI = `data:${mimeType};base64,${data}`;
    return dataURI;
}
exports.bytesToImageURI = bytesToImageURI;
function split_by_length(arr1, arr2, sum) {
    var result = Array();
    // console.log(arr2)
...
exports.split_by_length = split_by_length;
function parseDevices(out) {
    const s_length = out.split("\n")[1].split("  ").map((item) => item.length);
    var length_arr = [];
...
exports.parseDevices = parseDevices;
...
function findDevices() {
    return __awaiter(this, void 0, void 0, function* () {
        const stdout = yield child.execSync('frida-ls-devices');
...
function findProcesses(deviceId, appName) {
    return __awaiter(this, void 0, void 0, function* () {
        ...
        var foundProcess = [];
        if (appName.trim() !== "") {
            foundProcess = processes.filter(proc => { if (proc.name == appName) {
                return { "name": proc.name, "pid": proc.pid };
            } });
        }
        else {
...
function findApps(deviceId) {
    return __awaiter(this, void 0, void 0, function* () {
        ...
        try {
            ...
            // console.log(devices);
            const device = filtered[0];
            const applications = yield device.enumerateApplications({ scope: device_1.Scope.Full });
            for (const app in applications) {
                const appInfo = applications[app];
                const params = applications[app].parameters;
                if ((_a = params.icons) === null || _a === void 0 ? void 0 : _a.length) {
...
function compareByType(a, b) {
    ...
    if (a.type === 'user' && b.type !== 'user') {
        ...
        // If both have the same type or both are not 'user', maintain the original order
        return 0;
    }
}
function startApp(deviceId, appId) {
    return __awaiter(this, void 0, void 0, function* () {
        var tmpOutput = { status: false, output: null };
        try {
            const mgr = frida.getDeviceManager();
            const devices = yield mgr.enumerateDevices();
...
function attachApp(deviceId, processID) {
    return __awaiter(this, void 0, void 0, function* () {
