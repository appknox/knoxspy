...
class DBManager {
    ...
    clearActiveSession(callback: (status: boolean) => void) {
        const sql = `DELETE FROM current_session WHERE id>0`;
        this.db.run(sql, function(err: any) {
            if (err) {
    ...
    getRepeaterRequest(rowId: number = -1, callback: (data: any) => void) {
        console.log("Fetching repeater row", rowId);
        
        this.db.get(`SELECT * FROM repeater_traffic where id=${rowId}`, (err: any, rows: any) => {
            if (err) {
                console.error('Error querying database:', err.message);
                callback([]);
            } else {
    ...
    duplicateRepeater(rowId: any, callback: (lastObj: any) => void) {
        console.log("Repeating repeater", rowId);
        
        this.getActiveSession((rows: string) => {
            const tmpSession = JSON.parse(rows)
            this.getRepeaterRequest(rowId, (data: any) => {
                var tmpData = JSON.parse(data)
                delete tmpData.id
                //console.log(tmpData);
                const columns = Object.keys(tmpData);
    ...
    updateReplayedRepeater(row: any, callback: (lastObj: any) => void) {
        ...
        delete tmpData.id;
        tmpData['request_headers'] = JSON.stringify(tmpData['request_headers'])
        tmpData['response_headers'] = JSON.stringify(tmpData['response_headers'])
        console.log(tmpData);
    ...
    updateRepeaterTitle(rowId: any, title: string, callback: (lastObj: any) => void) {
        const tmpId = rowId;
        const tmpTitle = title;
        const tmpValues = [tmpTitle, tmpId];

        const sql = `UPDATE repeater_traffic set title=? where id=?`;
        //console.log(sql);
        
        this.db.run(sql, tmpValues, function(row: any, err: any) {
            if (err) {
                console.error(`Error updating data into repeater_traffic: ${err.message}`);
                callback({})
            } else {
    ...
    deleteRepeaterTab(rowId: any, callback: (status: any) => void) {
        const tmpId = rowId;
        const tmpValues = [tmpId];

        const sql = `DELETE FROM repeater_traffic WHERE id=?`;
        
        this.db.run(sql, tmpValues, function(row: any, err: any) {
            if (err) {
                console.error(`Error removing data from repeater_traffic: ${err.message}`);
                callback(false)
            } else {
                callback(true)
            }
    ...
    createNewLibrary(data: any, callback: (lastObj: any) => void) {
        var tmpData = JSON.parse(data)
        const columns = Object.keys(tmpData);
        const values = Object.values(tmpData);
        const placeholders = columns.map(() => '?').join(',');
        const sql = `INSERT INTO library (${columns.join(', ')}) VALUES (${placeholders})`;
        console.log(sql);
