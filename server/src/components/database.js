...
class DBManager {
    ...
    getRepeaterRequest(rowId = -1, callback) {
        console.log("Fetching repeater row", rowId);
        this.db.get(`SELECT * FROM repeater_traffic where id=${rowId}`, (err, rows) => {
            if (err) {
                console.error('Error querying database:', err.message);
                callback([]);
            }
    ...
    duplicateRepeater(rowId, callback) {
        console.log("Repeating repeater", rowId);
        this.getActiveSession((rows) => {
            const tmpSession = JSON.parse(rows);
            this.getRepeaterRequest(rowId, (data) => {
                var tmpData = JSON.parse(data);
                delete tmpData.id;
                //console.log(tmpData);
                const columns = Object.keys(tmpData);
    ...
    deleteRepeaterTab(rowId, callback) {
        const tmpId = rowId;
    ...
    createNewLibrary(data, callback) {
        var tmpData = JSON.parse(data);
        const columns = Object.keys(tmpData);
        const values = Object.values(tmpData);
        const placeholders = columns.map(() => '?').join(',');
