        ...
        <div id="app-wrapper">
            <Card style="width: 25rem; overflow: hidden">
                <template #content v-if="!isConnecting && currentSession.app.selectedApp">
                    ...
                    <p class="m-0" style="font-size: 20px; margin-bottom: 5px;"><b>{{ currentSession.app.selectedApp.name }}</b></p>
                    <p class="m-0 p-0" style="margin: 0;">{{ currentSession.app.selectedApp.id }}</p>
                ...
                <template #footer v-if="isSpawned && currentSession.app.selectedApp">
                    <div class="flex gap-4 mt-1" style="border-top: 1px solid #eee; padding-top: 20px;">
                        ...
                        <Button @click="actionHandler({ value: 'disconnect' })" label="Disconnect" severity="danger" class="w-full" style="display: none;"/>
                    </div>
                </template>
    ...
    methods: {
        ...
        actionHandler(event: any) {
            ...
            if(event.value === 'spawn' || event.value === 'attach') {
                this.startApp(this.currentSession.app.selectedApp.id, event.value + "App");
            }
        },
