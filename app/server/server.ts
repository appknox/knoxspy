...
async function compileFridaAgent(folderPath: string): Promise<void> {
  try {
    execSync(
      `frida-compile -o ${path.join(folderPath, "_agent.js")} ${folderPath}`,
      { timeout: 30000 }
    );
  } catch (error) {
...
function logRequest(req: Request, res: Response, next: NextFunction): void {
  ...
  console.log(`[${new Date().toISOString()}] ${method} request to ${endpoint}`);
  next();
}
...
// API Routes
router.post("/intruder_test", async (_req: Request, res: Response) => {
  return res.status(200).json({ status: true, message: "testing" });
});

router.post("/upload", upload.single, async (req: Request, res: Response) => {
  if (!req.file) {
  ...
  async (req: Request, res: Response) => {
    ...
    try {
      ...
      if (tmpAgentFiles.includes("package.json")) {
        ...
        const compiledFile = path.join(extractedDir, "_agent.js");
